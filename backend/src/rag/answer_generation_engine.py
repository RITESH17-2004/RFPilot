...
class AnswerGenerationEngine:
    ...
    def __init__(self, device: str = "cpu", executor: ThreadPoolExecutor = None):
        # Retrieve Sanitized Mistral API key from Config class
...
{response_guidance['template']}"""
    ...
    def _post_process_answer(self, answer: str, relevant_chunks: List[Tuple[Dict, float]]) -> str:
        ...
        Includes aggressive sanitization to remove characters like │ that cause crashes.
        """
        if not answer:
            return ""
        ...
        # Aggressive Sanitization: Keep ONLY printable ASCII (32-126) + newline/tab
        # This prevents the 'ascii' codec can't encode character error permanently.
