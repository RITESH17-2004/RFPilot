...
class DocumentTextExtractor:
    ...
    async def process_document(self, document_url: str, lang: str = 'eng') -> AsyncGenerator[Dict, None]:
        ...
        if document_url.startswith("file://"):
            local_path = document_url.replace("file://", "").replace("%20", " ")
            # Strip leading slash if on Windows and path starts with /C:/
            ...
            if local_path.lower().endswith('.txt'):
                ...
                text = await self._extract_pdf_local(local_path, lang=lang)
            elif local_path.lower().endswith(('.docx', '.doc')):
                with open(local_path, 'rb') as f:
                    text = self._extract_docx_text(BytesIO(f.read()))
            # ... add other local handlers if needed
            
            if text:
                logging.info("Local document parsing finished.")
    ...
    async def _extract_pdf_local(self, temp_path: str, lang: str = 'eng') -> str:
        """Extracts text from a local PDF file."""
        logging.info(f"Parsing local PDF: {temp_path}")
        text = ""
        loop = asyncio.get_event_loop()
        def parse_pdf():
            nonlocal text
            try:
                ...
                logging.error(f"Error parsing local PDF: {e}")
