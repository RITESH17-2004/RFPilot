                                    ...
                                    {filteredRFPs.map(rfp => (
                                        <tr key={rfp.id} className="hover:bg-slate-50/60 transition-all group">
                                            <td className="px-10 py-8">
                                                <div className="flex items-center gap-6">
                                                    <div className="w-14 h-14 bg-slate-50 rounded-2xl flex flex-col items-center justify-center border border-slate-100 group-hover:border-indigo-200 transition-colors">
                                                        ...
                                                        <span className="text-sm font-black text-slate-500 group-hover:text-indigo-600 transition-colors">00{rfp.id}</span>
                                                    </div>
                                                    <div>
                                                        <span className="font-black text-slate-800 text-base group-hover:text-indigo-600 transition-colors block mb-1">{rfp.title}</span>
                                            ...
                                            <td className="px-10 py-8 text-right">
                                                {rfp.status === 'draft' ? (
                                                    <Link href={`/bank/review/${rfp.id}`} className="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl active:scale-95">
                                                        <Edit3 size={16} /> Review & Publish
                                                    </Link>
                                                ) : (
                                                    <div className="flex items-center justify-end gap-3">
                                                        <a href={`${BACKEND_URL}${rfp.pdf_url}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-slate-50 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-2xl border border-slate-100 transition-all shadow-sm" title="Review PDF">
                                                            <Eye size={18} />
                                                        </a>
                                                        <Link href={`/bank/queries/${rfp.id}`} className="inline-flex items-center gap-2 px-5 py-3 bg-white text-slate-700 border border-slate-200 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:border-indigo-300 hover:text-indigo-600 transition-all shadow-sm active:scale-95">
                                                            <MessageSquare size={16} className="text-indigo-500" /> Q&A
                                                        </Link>
